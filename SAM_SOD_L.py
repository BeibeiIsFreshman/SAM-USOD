...
from Mine.encoder_adapter.image_encoder_weitiao1 import ImageEncoderViT
...
class MDSAM(nn.Module):
    ...
    def forward(self, img, depth=None):
        ...
        return (F.sigmoid(self.out_sam(low_res_masks)), F.sigmoid(self.out1_best(out[0])), F.sigmoid(self.out2_best(out[1])),
                F.sigmoid(self.out3_best(out[2])), F.sigmoid(self.out4_best(out[3])))
...
def reshapeRel(k, rel_pos_params, img_size):
    if not (('5' in k and '15' not in k and '25' not in k) or '11' in k or '17' in k or '23' in k):
...
if __name__ == '__main__':
    model = MDSAM().cuda()
    state = load(model,
                 "/media/tbb/9b281502-670b-4aec-957e-085adc101020/UAV/Fine_SAM/Backbone_pth/sam_vit_l_0b3195.pth", 256)
